...
def draw_debug_markers(hit_positions, radius=0.01):
    ...
    """
    if isinstance(hit_positions, th.Tensor):
        color = th.cat(
            [
                th.rand(3, device=hit_positions.device, dtype=hit_positions.dtype),
                th.ones(1, device=hit_positions.device, dtype=hit_positions.dtype),
            ]
        )
    else:
        color = th.cat([th.rand(3), th.ones(1)])
